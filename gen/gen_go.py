...
templ_gofunc = Template(
"""
${doc}
func ${code_self}${name}(${args}) ${rettype}{
    ${code_pre}
    ${code}
    ${code_post}
    ${ret}
...
""")
templ_cppfunc = Template(
"""${rettype} ${code_self}${name}(${args}) {
    ${code_pre}
    ${code}
    ${code_post}
    ${ret}
...
""")
templ_hppfunc = Template(
"""${rettype} ${code_self}${name}(${args});
""")

templ_gotest = Template(
"""
func TestMat${name}(t *testing.T) {
    src := NewMatWithSize(100, 100, MatTypeCV32F)
    dst := NewMat()
    ${code}
    if dst.Empty() {
        t.Error("Test${name} dst should not be empty.")
    }
}
""")
#
# mapping c++ types to cgo
#
templ_arg_size = Template("""
    c_${name} := C.struct_Size{
        width:  C.int(${name}.X),
        height: C.int(${name}.Y),
    }""")

templ_arg_string = Template("""
    c_${name} := C.CString(${name})
    defer C.free(unsafe.Pointer(c${name}))""")

templ_arg_scalar = Template("""
    c_${name} := C.struct_Scalar{
        val1: C.double(${name}.B),
        val2: C.double(${name}.G),
        val3: C.double(${name}.R),
        val4: C.double(${name}.A),
    }""")

templ_arg_contour = Template("""
    std::vector<cv::Point> pts_${name};
    for (size_t i = 0; i < ${name}.length; i++) {
        pts_${name}.push_back(cv::Point(${name}.points[i].x, ${name}.points[i].y));
    }""")

templ_arg_rect = Template("""
    c_${name} := C.struct_Rect{
        x:      C.int(${name}.Min.X),
        y:      C.int(${name}.Min.Y),
        width:  C.int(${name}.Size().X),
        height: C.int(${name}.Size().Y),
    }""")
...
class FuncInfo(object):
    ...
    # returns code_pre, arg
    def cgo_arg(self,name,tp):
        #print ("cgo_arg",name,tp)
        if tp=="Mat":
            return "",name+".p"
        if tp=="int" or tp=="double" or tp=="float" or tp=="bool":
            return "","C."+tp+"("+name+")"
        if tp=="Point" or tp=="Size":
            return templ_arg_size.substitute(name=name), "c_" + name
        if tp=="Rect":
            return templ_arg_rect.substitute(name=name), "c_" + name
        if tp=="String":
            return templ_arg_string.substitute(name=name), "c_" + name
        if tp=="Scalar":
            return templ_arg_scalar.substitute(name=name), "c_" + name
        if tp=="Contour":
            return templ_arg_contour.substitute(name=name), "pts_" + name
        return "", tp

    # returns code_post, name
    def cgo_ret(self,name,tp):
        if tp=="Mat":
            return "", "Mat{p:"+name+"}"
        if tp=="Rect":
            return "image.Rect(int("+name+".x), int("+name+".y), int("+name+".x+"+name+".width), int("+name+".y+"+name+".height))"
        return "", tp + "("+name+")"

    # returns code_pre, argname
    def cpp_arg(self,name,tp):
        if tp=="Mat":
            return "","*" + name
        if tp == "Point":
            return "cv::Point c_"+name+"("+name+".x, "+name+".y);", "c_" + name
        if tp=="Size":
            return "cv::Size c_"+name+"("+name+".width, "+name+".height);","c_" + name
        if tp == "Scalar":
            return "cv::Scalar c_"+name+"("+name+".val1, "+name+".val2, "+name+".val3, "+name+".val4);", "c_" + name
        return "", name

    def cpp_ret(self,name,tp):
        return "", name
    ...
    def gen_code(self, codegen):
        ...
        goname = self.name.capitalize()[0] + self.name[1:]
        cppname = "Mat_" + goname
        code = ""
        if self.classname:
            ...
            if not self.isconstructor:
                ...
                goname = selfinfo.wname + "." + goname

        all_code_variants = []
        declno = -1
        for v in self.variants:
            cgo_doc = "// " + v.docstring.replace("\n","\n// ")
            cgo_doc = cgo_doc.replace("@brief", goname)
            cgo_code_pre = ""
            cgo_code_post = ""
            cgo_code_ret = ""
            cgo_func_args = ""
            ...
            test_code = ""
            for a in v.args:
                #if a.defval: continue
                cgo_code_args += a.name + " " + self.go_arg(a.tp) + ", "
                cpp_code_args += a.tp + " " + a.name + ", "
                x,y = self.cgo_arg(a.name,a.tp)
                if x:
                    cgo_code_pre += x + "\n"
                cgo_func_args += y + ", "
                x,y = self.cpp_arg(a.name,a.tp)
                if x:
                    cpp_code_pre += x + "\n"
                cpp_func_args += y + ", "
                test_args += a.name + ", "
            cpp_code_args = cpp_code_args[:-2]
            cpp_func_args = cpp_func_args[:-2]
            cgo_code_args = cgo_code_args[:-2]
            cgo_func_args = cgo_func_args[:-2]
            test_args = test_args[:-2]
            test_code = goname + "(" + test_args + ")"
            if v.rettype:
                cgo_rettype = "("+v.rettype+") "
                x,y = self.cgo_ret("ret", v.rettype)
                cgo_ret = "return " + y
                cgo_code += "ret := "
                cpp_rettype = v.rettype
                x,y = self.cpp_ret("ret", v.rettype)
                cpp_ret = "return " + y + ";"
                cpp_code += v.rettype + " ret = "
            else:
                cpp_rettype = "void"
            cgo_code += "C." + cppname + "(" + cgo_func_args + ")"
            cpp_code += "cv::" + self.name + "(" + cpp_func_args + ");"

            code = templ_gofunc.substitute(doc=cgo_doc, code_self=cgo_code_self, name=goname, args=cgo_code_args, rettype=cgo_rettype, code_pre=cgo_code_pre, code=cgo_code, code_post=cgo_code_post, ret=cgo_ret)
            hpp  = templ_hppfunc.substitute(code_self=cpp_code_self, name=cppname, args=cpp_code_args, rettype=cpp_rettype)
            cpp  = templ_cppfunc.substitute(code_self=cpp_code_self, name=cppname, args=cpp_code_args, rettype=cpp_rettype, code_pre=cpp_code_pre, code=cpp_code, code_post=cpp_code_post, ret=cpp_ret)
            test = templ_gotest.substitute(name=goname, code=test_code)
...
class GoWrapperGenerator(object):
    def __init__(self):
        self.clear()
    ...
    def clear(self):
        ...
        self.code_cpp = StringIO()
        self.code_go = StringIO()
        self.code_test = StringIO()
        self.code_go.write(go_pre)
        self.code_test.write(test_pre)
        self.code_include.write(inc_pre)
        self.code_cpp.write(cpp_pre)
        self.class_idx = 0
    ...
    def gen(self, srcfiles, output_path):
        self.clear()
        self.parser = hdr_parser.CppHeaderParser(generate_umat_decls=False)
        ...
        for ns_name, ns in sorted(self.namespaces.items()):
            for name, func in sorted(ns.funcs.items()):
                if func.isconstructor:
                    continue
                code,hpp,cpp,test = func.gen_code(self)
                self.code_include.write(hpp)
                self.code_cpp.write(cpp)
                self.code_go.write(code)
                self.code_test.write(test)
                #print(code,hpp,cpp,test)
        ...
        #print(self.code_include.getvalue())
        #print(self.code_cpp.getvalue())
        #print(self.code_go.getvalue())
        #print(self.code_test.getvalue())
        # That's it. Now save all the files
        self.code_include.write(inc_post)
        self.save(output_path, "generated.h", self.code_include)
        self.save(output_path, "generated.cpp", self.code_cpp)
        self.save(output_path, "generated.go", self.code_go)
        self.save(output_path, "generated_test.go", self.code_test)
